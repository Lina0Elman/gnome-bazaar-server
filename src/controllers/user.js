...
exports.addUser = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .json({ message: "User added successfully", userId: user._id });
  } catch (err) {
  ...
  try {
    ...
    if (!purchases.length) {
      return res
        ...
        .json({ message: "No purchases found" });
    }
    ...
    const dataPreview = purchases.map((purchase) => {
      ...
      const totalQuantity = purchase.products.reduce(
        (sum, product) => sum + product.quantity,
        0
      );
      const productTitles = purchase.products
...
exports.getUserPurchases = async (req, res) => {
  ...
  try {
    // Find purchases by the user
    const userPurchases = await Purchase.find({ user: userId }).populate(
      "products.product"
    );
    ...
    const purchases = userPurchases.map((purchase) => ({
      ...
      products: purchase.products.map((item) => ({
        product: {
          ...
          img: item.product.img
            ? `data:image/webp;base64,${item.product.img.toString("base64")}`
            : null,
      ...
      signature: purchase.signature
        ? `data:image/webp;base64,${purchase.signature.toString("base64")}`
        : null,
    }));
...
exports.getUserCategories = async (req, res) => {
  ...
  try {
    ...
    const categories = Object.entries(categoryCounts).map(([title, value]) => ({
      title,
      value,
    }));
    ...
    if (cartItem) {
      ...
      if (cartItem.quantity >= product.quantity) {
        return res.status(StatusCodes.BAD_REQUEST).json({
          message: `Maximum available quantity for ${product.name} reached`,
        });
      }
  ...
  try {
    ...
    const cartWithImages = user.cart.map((item) => ({
      product: {
        ...
        price:
          item.product.price instanceof mongoose.Types.Decimal128
...
exports.sendCreditsToUser = async (req, res) => {
  ...
  if (typeof creditsToAdd !== "number" || creditsToAdd <= 0) {
    return res
      .status(StatusCodes.BAD_REQUEST)
      .json({ message: "Invalid credits value" });
  }
...
exports.getMediaFollowers = async (req, res) => {
  const twitterApiUrl =
    "https://api.twitter.com/2/users/by/username/Gnome_Bazaar1";
  ...
  try {
    ...
    // Fetch followers count using the user ID
    const followersResponse = await axios.get(
      `https://api.twitter.com/2/users/${userId}?user.fields=public_metrics`,
      {
        headers: {
          Authorization: `Bearer ${process.env.BEARER_TOKEN}`, // consistent use of the env variable
        },
      }
    );
    ...
    const followersCount =
      followersResponse.data.data.public_metrics.followers_count;
